...
export function SignalForm({
  ...
  useEffect(() => {
    if (vipStatus === 'PENDING') setWaitingMessage('Analisando cadastro para prioridade PREMIUM.');
    else if (vipStatus === 'AWAITING_DEPOSIT') setWaitingMessage('Cadastro ok! Aguardando depósito.');
    else if (vipStatus === 'DEPOSIT_PENDING') setWaitingMessage('Verificando confirmação de depósito.');
  ...
  return (
    <div className="w-full space-y-6 text-center">
      ...
      <div className="space-y-2">
        <p className="text-[0.65rem] text-foreground/40 font-bold uppercase tracking-[0.25em]">Configuração Operacional</p>
      ...
      <div className="space-y-5 text-left">
        <div className="space-y-2.5">
          <div className="flex justify-between items-center px-1">
            <Label className="text-[0.65rem] font-black text-muted-foreground uppercase tracking-widest opacity-60">Ativo:</Label>
            <div className="flex items-center space-x-2.5 bg-white/5 px-2.5 py-1 rounded-full border border-white/5">
              ...
              <Switch id="otc-switch" checked={showOTC} onCheckedChange={setShowOTC} disabled={isLoading} className="scale-50" />
            </div>
          </div>
          <Select value={formData.asset} onValueChange={(value) => setFormData({ ...formData, asset: value as Asset })} disabled={isLoading}>
            <SelectTrigger className="h-12 text-sm rounded-xl bg-white/5 border-white/5 hover:bg-white/10 transition-all">
              <SelectValue asChild>
        ...
        <div className="space-y-2.5">
          <Label className="text-[0.65rem] font-black text-muted-foreground uppercase tracking-widest opacity-60 px-1">Expiração:</Label>
      ...
      <div className="w-full space-y-4 pt-4">
          <Button
            size="lg"
            className="w-full h-14 text-base font-black bg-primary text-primary-foreground shadow-2xl transition-all pulse-strong rounded-xl uppercase tracking-tighter"
            onClick={onSubmit}
