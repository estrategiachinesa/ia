...
import { BarChart, Loader2, Lock, Timer, Crown, ExternalLink, Radio, Circle } from 'lucide-react';
...
export function SignalForm({
  ...
  // Lógica do cronómetro de abertura
  useEffect(() => {
    ...
    <div className="w-full h-full flex flex-col justify-between p-0 overflow-hidden text-center bg-black/20">
      <div className="flex flex-col gap-1.5 flex-grow overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-2 shrink-0 px-4 pt-2">
            <Button asChild variant="ghost" size="sm" className="h-9 md:h-10 text-[0.6rem] font-black uppercase tracking-widest border border-white/5 bg-white/5 rounded-xl hover:bg-primary/20 hover:border-primary transition-all duration-300">
        ...
        {!isMarketOpen && !showOTC && nextOpenText && (
            <div className="mx-4 p-2 bg-red-600/10 border border-red-500/20 rounded-xl flex items-center justify-center gap-4 animate-in fade-in duration-500">
                <div className="flex items-center gap-2">
                    <Circle className="h-2 w-2 fill-red-500 text-red-500 animate-pulse" />
                    <span className="text-[0.65rem] font-black text-red-500 uppercase tracking-tight">Mercado fechado</span>
                </div>
                <span className="text-[0.65rem] font-bold text-zinc-400 uppercase tracking-tighter">
                    O ativo abre em <span className="text-white font-mono">{nextOpenText}</span>
                </span>
            </div>
        )}

        {waitingMessage && (
            <Alert className="text-center bg-primary/5 border-primary/20 py-1 rounded-xl mx-4 shrink-0">
        ...
        <div className="space-y-4 flex-grow flex flex-col justify-center px-6">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center px-1">
              <Label className="text-[0.55rem] font-black text-muted-foreground uppercase tracking-widest opacity-60">Par de Moedas:</Label>
              <div className="flex items-center space-x-1.5 bg-black/40 px-2 py-0.5 rounded-full border border-white/5">
                <Label htmlFor="otc-switch" className="text-[0.45rem] font-black text-muted-foreground uppercase opacity-70">OTC</Label>
            ...
            <Select value={formData.asset} onValueChange={(value) => setFormData({ ...formData, asset: value as Asset })} disabled={isLoading}>
              <SelectTrigger className="h-11 md:h-12 rounded-xl bg-white/5 border-white/10 hover:border-primary hover:bg-white/10 transition-all duration-300">
                <SelectValue asChild>
                  <div className="flex items-center justify-center gap-3 w-full">
                    <CurrencyFlags asset={formData.asset} />
                    <span className="font-black tracking-tight text-[0.8rem] md:text-sm uppercase">{formData.asset}</span>
                  </div>
                </SelectValue>
              </SelectTrigger>
                ...
                {assets.map(asset => (
                  <SelectItem key={asset} value={asset} className="rounded-lg focus:bg-primary/20 py-2.5">
                    <div className="flex items-center gap-3 w-full justify-center">
                        ...
                        <span className="font-black text-[0.7rem] uppercase">{asset}</span>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {/* EXPIRAÇÃO */}
          <div className="space-y-1.5">
            <Label className="text-[0.55rem] font-black text-muted-foreground uppercase tracking-widest opacity-60">Tempo de Expiração:</Label>
            <Select value={formData.expirationTime} onValueChange={(value) => setFormData({ ...formData, expirationTime: value as ExpirationTime })} disabled={isLoading}>
              <SelectTrigger className="h-11 md:h-12 rounded-xl bg-white/5 border-white/10 text-[0.8rem] font-black uppercase text-center hover:border-primary hover:bg-white/10 transition-all duration-300">
                <SelectValue placeholder="Tempo" />
              </SelectTrigger>
              <SelectContent className="rounded-xl border-white/10 bg-black/95 backdrop-blur-2xl">
                <SelectItem value="1m" className="rounded-lg font-black py-2.5 text-[0.7rem] uppercase text-center">M1 (60 Segundos)</SelectItem>
                <SelectItem value="5m" className="rounded-lg font-black py-2.5 text-[0.7rem] uppercase text-center">M5 (5 Minutos)</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
      ...
      <div className="w-full space-y-2 pt-2 pb-4 shrink-0 px-6">
          <Button
            size="lg"
            className="w-full h-12 md:h-14 text-sm md:text-base font-black bg-primary text-primary-foreground shadow-2xl shadow-primary/20 rounded-xl uppercase tracking-tighter hover:scale-[1.02] hover:shadow-primary/40 active:scale-95 transition-all duration-300 shine-effect"
            onClick={onSubmit}
          ...
          {!isPremium && (
            <Button variant="link" className="w-full h-auto text-purple-400 py-0.5 hover:text-purple-300 transition-colors" onClick={() => {
              if (vipStatus) setVipModalOpen(true);
                ...
                <span className="text-[0.6rem] font-black uppercase tracking-widest">DESBLOQUEAR ACESSO PREMIUM</span>
            </Button>
          )}
      </div>
    </div>
